...
import { createOffer, feePercentage, formatLtc, getEscrow, getListing, getOffer, getUser, saveEscrow, saveOffer, storageMessage } from "../marketplace.js";
...
composer.callbackQuery(/^purchase:init:(.+)$/, async (ctx) => {
  ...
  const offer = await createOffer(ctx, listing, fee);
  if (!offer) { await ctx.reply(storageMessage()); return; }
  await ctx.reply(`Payment amount: ${formatLtc(offer.amount)} LTC.\nSeller fee: ${offer.feePercentage}% (${formatLtc(offer.feeAmount)} LTC).\nEstimated seller payout: ${formatLtc(offer.sellerPayoutAmount)} LTC.\n${networkFeeNote}\n\nEscrow payment monitoring isn't connected yet, so no payment address can be issued. The owner needs to connect the external LTC escrow service.`, { reply_markup: inlineKeyboard([[inlineButton("Raise dispute", `dispute:start:${offer.id}`)]]) });
  const owner = adminChatId(ctx as unknown as { env?: Record<string, unknown> });
  if (owner) await notify(ctx, owner, `A buyer started a purchase for ${listing.title}. Escrow: ${formatLtc(offer.amount)} LTC. Seller fee: ${offer.feePercentage}% (${formatLtc(offer.feeAmount)} LTC). Verify payment externally before marking it paid.`, inlineKeyboard([[inlineButton("Mark payment received", `offer:markpaid:${offer.id}`)]]));
});
...
composer.callbackQuery(/^offer:markpaid:(.+)$/, async (ctx) => {
  ...
  await notify(ctx, offer.buyerChatId, `Your LTC payment of ${formatLtc(offer.amount)} LTC is confirmed in escrow. Seller fee: ${offer.feePercentage}%. Confirm delivery when you receive the card.`, inlineKeyboard([[inlineButton("Confirm received", `purchase:received:${offer.id}`), inlineButton("Raise dispute", `dispute:start:${offer.id}`)]]));
  if (listing) await notify(ctx, listing.sellerChatId, `LTC payment is confirmed in escrow. Your estimated payout is ${formatLtc(offer.sellerPayoutAmount)} LTC after the ${offer.feePercentage}% seller fee. Ship the card, then wait for buyer confirmation.`);
...
composer.callbackQuery(/^purchase:received:(.+)$/, async (ctx) => {
  ...
  const escrow = await getEscrow(ctx, offer.escrowId); const listing = await getListing(ctx, offer.listingId);
  ...
  escrow.feeDeducted = offer.feeAmount; escrow.releaseStatus = "released"; offer.status = "released";
  await saveEscrow(ctx, escrow); await saveOffer(ctx, offer);
  await ctx.reply(`Receipt confirmed. ${formatLtc(offer.sellerPayoutAmount)} LTC is approved for the seller after the ${offer.feePercentage}% fee (${formatLtc(offer.feeAmount)} LTC).`);
  await notify(ctx, seller.chatId, `The buyer confirmed receipt. Your payout of ${formatLtc(offer.sellerPayoutAmount)} LTC is approved for manual release after the ${offer.feePercentage}% seller fee (${formatLtc(offer.feeAmount)} LTC).`);
  await notify(ctx, adminChatId(ctx as unknown as { env?: Record<string, unknown> }), `A seller payout of ${formatLtc(offer.sellerPayoutAmount)} LTC is approved for manual release after the ${offer.feePercentage}% seller fee (${formatLtc(offer.feeAmount)} LTC).`);
});
